...
activity task prompt (`prompt.v2`, `prompt.v3`, which adds a scene and a model dialogue to explanation, conversation and game activities, and `prompt.v4`, which teaches meaning before production and grades generously) asks the teacher to pace by it without
